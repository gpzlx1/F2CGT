    ...
    for i in range(3):
        ...
        if len(killed_pids) == 0:
            ...
            for pid in killed_pids:
                print("kill process {} on {}:{}".format(pid, ip, port),
                      flush=True)
                kill_cmd = ("ssh -o StrictHostKeyChecking=no -p " + str(port) +
...
def construct_torch_dist_launcher_cmd(
    ...
    Returns:
        cmd_str.
    """
    torch_cmd_template = ("-m torch.distributed.run "
...
def wrap_udf_in_torch_dist_launcher(
    ...
    new_udf_command = udf_command.replace(python_bin,
                                          f"{python_bin} {torch_dist_cmd}")
...
def construct_dgl_server_env_vars(
    ...
    """
    server_env_vars_template = ("DGL_ROLE={DGL_ROLE} "
...
def construct_dgl_client_env_vars(
    ...
    """
    client_env_vars_template = ("DGL_DIST_MODE={DGL_DIST_MODE} "
                                "DGL_ROLE={DGL_ROLE} "
                                "DGL_NUM_SAMPLER={DGL_NUM_SAMPLER} "
                                "DGL_NUM_CLIENT={DGL_NUM_CLIENT} "
                                "DGL_CONF_PATH={DGL_CONF_PATH} "
...
def submit_jobs(args, udf_command, dry_run=False):
    ...
    for i in range(len(hosts) * server_count_per_machine):
        ...
        cmd = (wrap_cmd_with_extra_envvars(cmd, args.extra_envs)
    ...
    client_env_vars = construct_dgl_client_env_vars(
        ...
        num_servers=args.num_servers,
        graph_format=args.graph_format,
        num_omp_threads=os.environ.get("OMP_NUM_THREADS",
                                       str(args.num_omp_threads)),
        group_id=0,
        pythonpath=os.environ.get("PYTHONPATH", ""),
    )
    ...
    for node_id, host in enumerate(hosts):
        ...
        cmd = wrap_cmd_with_local_envvars(torch_dist_udf_command,
                                          client_env_vars)
        cmd = (wrap_cmd_with_extra_envvars(cmd, args.extra_envs)
               if len(args.extra_envs) > 0 else cmd)
        cmd = "cd " + str(args.workspace) + "; " + cmd
...
def main():
    ...
    parser.add_argument(
        ...
        default="",
        help=
        "Optional. When issuing commands (via ssh) to cluster, use the provided username in the ssh cmd. "
    ...
    assert (args.num_trainers is not None and args.num_trainers
            > 0), "--num_trainers must be a positive number."
    assert (args.num_samplers is not None and args.num_samplers
            >= 0), "--num_samplers must be a non-negative number."
    assert (args.num_servers is not None and args.num_servers
            > 0), "--num_servers must be a positive number."
    assert (args.num_server_threads
            > 0), "--num_server_threads must be a positive number."
    assert (args.workspace
            is not None), "A user has to specify a workspace with --workspace."
    assert (

...
// packbits
// note: "Only int8_t, int64_t and bool are supported as an integral argument
...
torch::Tensor packbits(torch::Tensor tensor, int64_t dim, int64_t mask) {
  ...
  for (size_t outer_block = 0; outer_block < full_len;
       outer_block += block_stride, baseptr += block_stride,
    ...
    for (size_t e = 0; e < packed_size; e++) {
      ...
      if (orig_end > orig_shape) orig_end = orig_shape;
      ...
      while (orig_idx < orig_end) {
        ...
        for (; orig_ptr != orig_end; orig_ptr += 1, dst_ptr += 1) {

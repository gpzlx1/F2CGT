...
def kmeans(
    ...
    while True:
        ...
        for i in range(math.ceil(len(X) / batch_size)):
            ...
            dis[i * batch_size:i * batch_size +
                batch_size] = pairwise_distance_function(
                    X[i * batch_size:i * batch_size + batch_size],
                    initial_state)
        # dis =
        ...
        for index in range(num_clusters):
            ...
            selected = torch.nonzero(
                choice_cluster == index).squeeze(1).to(device)
    ...
    for i in tqdm.trange(math.ceil(len(X) / batch_size), mininterval=5):
        dis[:min(batch_size,
                 len(X) - batch_size * i)] = pairwise_distance_function(
                     X[i * batch_size:i * batch_size + batch_size].to(device),
                     initial_state, device)
        # print(dis.shape, choice_cluster[i*batch_size:i*batch_size+batch_size].shape)
        choice_cluster[i * batch_size:i * batch_size +
                       batch_size] = torch.argmin(
                           dis[:min(batch_size,
                                    len(X) - batch_size * i)],
                           dim=1)
    for index in range(num_clusters):
